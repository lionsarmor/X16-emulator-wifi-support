...
bool has_wifi_card = false;
uint16_t wifi_card_addr;
...
usage()
{
	...
	printf("-wifi [<address>]\n");
	printf("\tInstall the WiFi/ESP32 921.6Kbps Serial & Network card at the specified\n");
	printf("\taddress, or at $9FE0 by default (the card's factory IO7-Low setting).\n");
	printf("\tThe network UART (offset 0-7 of the card) is bridged to a built-in,\n");
	printf("\tZimodem-compatible AT-command modem that uses the host's real network\n");
	printf("\tconnection, so X16 software written for the real ESP32 network card\n");
	printf("\t(ATD\"host:port\" to dial, ATA<port> to listen, +++ to escape, etc.)\n");
	printf("\tworks against the real Internet. The card's physical RS-232 UART\n");
	printf("\t(offset 8-15) is emulated at the register level only; nothing is\n");
	printf("\tconnected to it yet.\n");
#ifdef TRACE
...
main(int argc, char **argv)
{
	...
	while (argc > 0) {
		if (!strcmp(argv[0], "-rom")) {
			...
			has_wifi_card = true;
			if (argc && argv[0][0] != '-') {
				wifi_card_addr = 0x9f00 | ((uint16_t)strtol(argv[0], NULL, 16) & 0xff);
				wifi_card_addr &= 0xfff0;
				argc--;
				argv++;
			} else {
				wifi_card_addr = 0x9fe0;
			}
...
//		}
		...
		if (video_get_irq_out() || via1_irq() || (has_via2 && via2_irq()) || (ym2151_irq_support && YM_irq()) || (has_midi_card && midi_serial_irq()) || (has_wifi_card && wifi_serial_irq())) {
//			printf("IRQ!\n");
			irq6502();
		}
